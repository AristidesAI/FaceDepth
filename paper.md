...
The pipeline runs on one machine with no cluster. Teacher labeling of 30,000 images takes about 18 hours at 0.48 images per second. Training ran 17,000 steps at roughly 3 seconds per step across the three resolutions. A plateau detector ends the run once three consecutive validations fail to improve, and a health watchdog polls disk, memory pressure, and training liveness every 30 seconds, then signals the trainer to checkpoint and exit if the machine degrades. That watchdog fired once by design at the end of the run and saved cleanly. Checkpoints write every five minutes, which bounds crash loss.
